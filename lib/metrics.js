...
const MAX_LOCAL_OPERATIONS_LIST_LENGTH = 5000;
...
exports.redis = async ({continuity, ledgerNode, ledgerNodeId}) => {
  ...
  if(continuity.localOpsListLength > MAX_LOCAL_OPERATIONS_LIST_LENGTH) {
    brCooldown.set(
      'ledger-consensus-continuity-stats-monitor:local-ops-list-length', {
      module: 'ledger-consensus-continuity-stats-monitor',
      message: `Current continuity local operations list length ` +
        `"${continuity.localOpsListLength} ops" exceeded maximum ` +
        `"${MAX_LOCAL_OPERATIONS_LIST_LENGTH}" ops.`
    });
  }
};
